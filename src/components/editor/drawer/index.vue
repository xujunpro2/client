    ...
    <nav class="navigation">
        <div class="tab-bar">
            <a title="Elements" class="tab" @click="toggleTab('elements')">
                <svg-icon icon-class="widgets" style="font-size: 24px; "
                    :color="showElements ? '#36a3f7' : 'rgba(0,0,0,.38)'" />
            </a>
            <a title="Settings" class="tab" @click="toggleTab('settings')">
                <svg-icon icon-class="settings" style="font-size: 24px; "
                    :color="showSettings ? '#36a3f7' : 'rgba(0,0,0,.38)'" />
            </a>
            <a title="Pages" class="tab" @click="toggleTab('pages')">
                <svg-icon icon-class="page" style="font-size: 24px; "
                    :color="showPages ? '#36a3f7' : 'rgba(0,0,0,.38)'" />
            </a>
            <!--Tab头下的蓝色指示状态条-->
            <div class="tab-indicator" :style="{'left': indiLeft, 'right': indiRight}"></div>
        </div>
